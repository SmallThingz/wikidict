...
`python3 tools/compress_blobs.py FILE.wikblb ...` writes verified `.wikblb.xz` files using XZ extreme (`-9e`) with independent 1 MiB blocks, retaining raw originals. It checks a streaming SHA-256 round trip before atomic publication. `--block-size` accepts 64 KiB through 16 MiB. The previous fast preset, on a 30-entry, 3,365,898-byte fixture, produced 371,560 bytes in a 0.070 s compression-and-verification smoke run; this is not a corpus benchmark.
