...
const upat = @import("scribunto_ustring_pattern.zig");
...
fn uLen(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const source = try sourceArg(args);
    return one(a, .{ .number = @floatFromInt(try countCodepoints(source)) });
}

fn uSub(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const source = try sourceArg(args);
...
fn uChar(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
...
fn uCodepoint(_: ?*anyopaque, _: *anyopaque, args: []const Value, _: std.mem.Allocator) ![]const Value {
    const source = try sourceArg(args);
    const len: i64 = @intCast(try countCodepoints(source));
    var first = normalizeIndex(if (args.len > 1 and args[1] != .nil) try integer(args[1]) else 1, len);
...
fn gcodepointNext(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, a: std.mem.Allocator) ![]const Value {
...
fn uGcodepoint(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const source = try sourceArg(args);
    const len: i64 = @intCast(try countCodepoints(source));
    var first = normalizeIndex(if (args.len > 1 and args[1] != .nil) try integer(args[1]) else 1, len);
    var last = normalizeIndex(if (args.len > 2 and args[2] != .nil) try integer(args[2]) else -1, len);
    ...
    return one(a, try rt.newNative(a, ctx, gcodepointNext));
}
...
fn uIsUtf8(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
...
fn uByteoffset(_: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const source = try sourceArg(args);
...
fn uUpper(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    return uCase(ctx_raw, args, a, true);
}

fn uLower(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    return uCase(ctx_raw, args, a, false);
}

fn uNormalize(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
...
fn uFind(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
...
fn uMatch(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    if (args.len < 2) return error.MissingArgument;
    const normalizer: *Normalizer = @ptrCast(@alignCast(ctx_raw.?));
    const source = try stringArg(a, args[0]);
    const pat = try stringArg(a, args[1]);
...
fn uGmatchNext(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, _: std.mem.Allocator) ![]const Value {
...
fn uGmatch(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    if (args.len < 2) return error.MissingArgument;
    const normalizer: *Normalizer = @ptrCast(@alignCast(ctx_raw.?));
    const source = try stringArg(a, args[0]);
    const pat = try stringArg(a, args[1]);
    ...
    return one(a, try rt.newNative(a, ctx, uGmatchNext));
}
...
fn replacementValue(vm: *exec.Vm, replacement: Value, search: *const upat.Search, m: upat.Match, a: std.mem.Allocator) ![]const u8 {
...
fn uGsub(ctx_raw: ?*anyopaque, vm_raw: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    ...
    if (replacement != .string and replacement != .table and replacement != .closure and replacement != .native)
    ...
    while (count < max_count and next_start <= search.source.codepoints.len) {
        ...
        switch (replacement) {
            ...
            else => try out.appendSlice(std.heap.smp_allocator, try replacementValue(vm, replacement, &search, m, a)),
        }
        count += 1;
        cursor = m.end;
        if (anchored) break;
...
fn setNative(a: std.mem.Allocator, table: *rt.Table, name: []const u8, call: rt.NativeCall) !void {
    try table.rawSet(a, .{ .string = name }, try rt.newNative(a, null, call));
}
fn setNativeCtx(a: std.mem.Allocator, table: *rt.Table, name: []const u8, ctx: ?*anyopaque, call: rt.NativeCall) !void {
    try table.rawSet(a, .{ .string = name }, try rt.newNative(a, ctx, call));
}

pub fn install(a: std.mem.Allocator, table: *rt.Table) !void {
    try setNative(a, table, "isutf8", uIsUtf8);
    try setNative(a, table, "byteoffset", uByteoffset);
    try setNative(a, table, "len", uLen);
    try setNative(a, table, "sub", uSub);
    try setNative(a, table, "char", uChar);
    try setNative(a, table, "codepoint", uCodepoint);
    try setNative(a, table, "gcodepoint", uGcodepoint);
    const normalizer = try createNormalizer(a);
    try setNativeCtx(a, table, "upper", normalizer, uUpper);
    try setNativeCtx(a, table, "lower", normalizer, uLower);
    try setNativeCtx(a, table, "find", normalizer, uFind);
    try setNativeCtx(a, table, "match", normalizer, uMatch);
    try setNativeCtx(a, table, "gmatch", normalizer, uGmatch);
    try setNativeCtx(a, table, "gsub", normalizer, uGsub);
    inline for (.{
        ...
        try setNativeCtx(a, table, item[0], ctx, uNormalize);
    }
}
...
test "utf8proc canonical normalization" {
    ...
    const nfd = try uNormalize(&nfd_ctx, @ptrCast(&dummy), &.{.{ .string = "é" }}, a);
    try std.testing.expectEqualStrings("e\u{301}", nfd[0].string);
    var nfc_ctx = NormalizeCtx{ .normalizer = normalizer, .options = (1 << 1) | (1 << 3) };
    const nfc = try uNormalize(&nfc_ctx, @ptrCast(&dummy), &.{.{ .string = "e\u{301}" }}, a);
...
test "Scribunto Unicode pattern functions operate on codepoints" {
    ...
    const ustring = try rt.newTable(a);
    try install(a, ustring);

    const match_fn = ustring.rawGet(.{ .string = "match" }).?;
    const matched = try vm.callValue(match_fn, &.{ .{ .string = "ʃə" }, .{ .string = "^." } });
    defer exec.Vm.freeResults(matched);
    try std.testing.expectEqualStrings("ʃ", matched[0].string);

    const find_fn = ustring.rawGet(.{ .string = "find" }).?;
    const found = try vm.callValue(find_fn, &.{ .{ .string = "αβ 12" }, .{ .string = "β%s(%d+)" } });
    ...
    const replaced = try vm.callValue(gsub_fn, &.{ .{ .string = "ʃə" }, .{ .string = "^." }, .{ .string = "" } });
    defer exec.Vm.freeResults(replaced);
