...
fn mmapPath(path: []const u8) !Mapped {
    ...
    var f: std.Io.File = .{ .handle = fd, .flags = .{ .nonblocking = false } }; defer f.close(io);
...
const Reducer = struct {
    ...
    fn addRoot(self: *Reducer, fields: []const []const u8, w: *std.Io.Writer) !void {
        ...
        if (isDynamicTarget(target)) {
            ...
            try w.writeAll("Y");
            for (fields[1..]) |field| { try w.writeByte('\t'); try w.writeAll(field); }
            try w.writeByte('\n');
            return;
        }
        const gop = try self.roots.getOrPut(self.allocator, target);
        if (!gop.found_existing) {
            const st = try self.allocator.create(RootState); st.* = .{}; gop.value_ptr.* = st;
        }
        ...
        for (args.items, 0..) |arg, arg_index| {
            ...
            for (args.items[arg_index + 1 ..]) |later| {
                if (std.mem.eql(u8, later.key, arg.key)) { shadowed = true; break; }
            }
    ...
    fn writeRoots(self: *Reducer, w: *std.Io.Writer) !void {
        const names = try self.allocator.alloc([]const u8, self.roots.count()); defer self.allocator.free(names);
        var i: usize = 0; var rit = self.roots.iterator();
        while (rit.next()) |e| : (i += 1) names[i] = e.key_ptr.*;
        std.mem.sort([]const u8, names, {}, lessStr);
        for (names) |name| {
            const st = self.roots.get(name).?;
            try w.writeAll("A\t"); try w.writeAll(name); try w.print("\t{d}\t{}\n", .{ st.calls, st.dynamic_key });
            const pnames = try self.allocator.alloc([]const u8, st.params.count()); defer self.allocator.free(pnames);
            i = 0; var pit = st.params.iterator(); while (pit.next()) |e| : (i += 1) pnames[i] = e.key_ptr.*;
            std.mem.sort([]const u8, pnames, {}, lessStr);
            for (pnames) |pname| {
                const ps = st.params.get(pname).?;
                try w.writeAll("V\t"); try w.writeAll(name); try w.writeByte('\t'); try w.writeAll(pname);
                ...
                if (!ps.top) {
                    ...
                    for (vals) |v| { try w.writeByte('\t'); try w.writeAll(v); }
                }
                try w.writeByte('\n');
            }
        }
...
fn isDynamicTarget(s: []const u8) bool { return std.mem.indexOf(u8, s, "{{") != null or std.mem.indexOf(u8, s, "[[") != null; }
fn isDynamicKey(s: []const u8) bool { return std.mem.indexOf(u8, s, "{{") != null or std.mem.indexOf(u8, s, "[[") != null; }
fn isDynamicValue(s: []const u8) bool { return std.mem.indexOf(u8, s, "{{") != null or std.mem.indexOf(u8, s, "[[") != null; }
...
fn parseArg(a: std.mem.Allocator, raw: []const u8, positional: *usize) !Arg {
    ...
    const tmp = try std.fmt.bufPrint(&buf, "{d}", .{positional.*}); positional.* += 1;
...
pub fn main(init: std.process.Init) !void {
    ...
    var reducer = Reducer{ .allocator = a }; defer reducer.deinit();
    ...
    var pos: usize = 0; var lines: u64 = 0; var released: usize = 0;
    while (pos < mapped.bytes.len) {
        const nl = std.mem.indexOfScalarPos(u8, mapped.bytes, pos, '\n') orelse mapped.bytes.len;
        const line = mapped.bytes[pos..nl]; pos = @min(nl + 1, mapped.bytes.len); lines += 1;
        if (line.len == 0) continue;
        if (line[0] == 'Q') {
            var fields: std.ArrayList([]const u8) = .empty; defer fields.deinit(a);
            try splitFields(a, line, &fields); try reducer.addRoot(fields.items, w);
        } else {
            try w.writeAll(line); try w.writeByte('\n'); reducer.passthrough += 1;
        }
        if (pos - released >= 128 * 1024 * 1024) {
            const ps = std.heap.page_size_min; const end = (pos / ps) * ps;
            if (end > released) {
    ...
    try reducer.writeRoots(w); try w.flush();
