...
fn check(source: []const u8, expected: []const f64) !Stats {
    ...
    _ = try @import("vm_flow.zig").run(allocator, &program);
    _ = try shape_opt.run(allocator, &program);
    const stats = try run(allocator, &program);
    ...
    for (out, expected) |value, number| {
        try std.testing.expect(value == .number);
        try std.testing.expectEqual(number, value.number);
    }
    return stats;
}

test "immutable captured table fields become slots" {
    const stats = try check(
        "local t={};t.x=2;local function f()return t.x end;return f()",
        &.{2},
    );
    try std.testing.expectEqual(@as(u64, 1), stats.captured_tables);
    try std.testing.expectEqual(@as(u64, 1), stats.slot_reads);
    try std.testing.expectEqual(@as(u64, 1), stats.slot_writes);
}
test "captured table identity reassignment stays dynamic" {
    const stats = try check(
        "local t={};t.x=1;local function replace()local n={};n.x=2;t=n end;local function get()return t.x end;replace();return get()",
        &.{2},
    );
    try std.testing.expectEqual(@as(u64, 0), stats.captured_tables);
    try std.testing.expectEqual(@as(u64, 0), stats.slot_reads);
}

test "descendant upvalue mutation invalidates captured shape" {
    const stats = try check(
        "local t={};t.x=1;local function outer()local function replace(v)t=v end;local n={};n.x=3;replace(n)end;local function get()return t.x end;outer();return get()",
        &.{3},
    );
    try std.testing.expectEqual(@as(u64, 0), stats.captured_tables);
    try std.testing.expectEqual(@as(u64, 0), stats.slot_reads);
...
test "captured aliases preserve one proven shape across branches" {
    const stats = try check(
        "local t={};t.x=5;local function f(flag)local a=t;if flag then a=t end;return a.x end;return f(true),f(false)",
        &.{ 5, 5 },
    );
    try std.testing.expectEqual(@as(u64, 1), stats.captured_tables);
    try std.testing.expectEqual(@as(u64, 1), stats.slot_reads);
}
test "captured slots preserve nil deletion and index metamethods" {
    const stats = try check(
        "local t={};t.x=1;setmetatable(t,{__index=function()return 9 end});local function f()return t.x end;t.x=nil;return f()",
        &.{9},
    );
    try std.testing.expectEqual(@as(u64, 1), stats.captured_tables);
    try std.testing.expect(stats.slot_reads >= 1);
    try std.testing.expect(stats.slot_writes >= 2);
}
