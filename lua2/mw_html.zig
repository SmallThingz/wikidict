...
fn setNative(node: *Node, name: []const u8, call: rt.NativeCall) !void {
    try node.table.rawSet(node.html.allocator, .{ .string = name }, try rt.newNative(node.html.allocator, node, call));
}

fn installNodeMethods(node: *Node) !void {
    try setNative(node, "tag", tagCall);
    try setNative(node, "done", doneCall);
    try setNative(node, "allDone", allDoneCall);
    try setNative(node, "wikitext", wikitextCall);
    try setNative(node, "node", nodeCall);
    try setNative(node, "css", cssCall);
    try setNative(node, "cssText", cssTextCall);
    try setNative(node, "addClass", addClassCall);
    try setNative(node, "attr", attrCall);
    try setNative(node, "newline", newlineCall);
    const mt = try rt.newTable(node.html.allocator);
    try mt.rawSet(node.html.allocator, .{ .string = "__tostring" }, try rt.newNative(node.html.allocator, node, tostringCall));
    node.table.metatable = mt;
}

fn createCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    ...
    const node = try newNode(html, null, tag_name);
    try installNodeMethods(node);
    return one(a, .{ .table = node.table });
}
fn tagCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
    if (args.len < 2 or args[1] != .string) return error.HtmlTagExpected;
    const child = try newNode(node.html, node, args[1].string);
    try installNodeMethods(child);
    try node.children.append(node.html.allocator, .{ .node = child });
    return one(a, .{ .table = child.table });
}

fn doneCall(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
    return one(a, .{ .table = if (node.parent) |parent| parent.table else node.table });
}

fn allDoneCall(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, a: std.mem.Allocator) ![]const Value {
    var node: *Node = @ptrCast(@alignCast(ctx_raw.?));
    while (node.parent) |parent| node = parent;
    return one(a, .{ .table = node.table });
}

fn wikitextCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
    for (args[1..]) |value| {
...
fn nodeCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
...
fn cssCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
...
fn cssTextCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
    if (args.len >= 2 and args[1] != .nil) try node.css_text.append(node.html.allocator, try scalarText(node.html.allocator, args[1]));
    return returnSelf(node, a);
}
fn addClassCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
...
fn attrCall(ctx_raw: ?*anyopaque, _: *anyopaque, args: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
...
fn newlineCall(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
...
fn tostringCall(ctx_raw: ?*anyopaque, _: *anyopaque, _: []const Value, a: std.mem.Allocator) ![]const Value {
    const node: *Node = @ptrCast(@alignCast(ctx_raw.?));
