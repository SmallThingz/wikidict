...
pub fn main(init: std.process.Init) !void {
    ...
    var dir = try std.Io.Dir.cwd().openDir(init.io, args[1], .{ .iterate = true }); defer dir.close(init.io);
    var it = dir.iterate();
    var out_buf: [256 * 1024]u8 = undefined;
    var stdout = std.Io.File.stdout().writer(init.io, &out_buf); const w = &stdout.interface;
    var modules: usize = 0; var exports: usize = 0; var functions: usize = 0; var dynamic: usize = 0;
    while (try it.next(init.io)) |entry| {
        ...
        modules += 1; functions += b.functions.count(); if (b.dynamic_top_level) dynamic += 1;
        ...
        switch (b.return_binding) {
            .table => |table| {
                ...
                while (fit.next()) |field| switch (field.value_ptr.*) {
                    .function => |fn_id| {
                        try w.writeAll("E\t"); try w.writeAll(id); try w.writeByte('\t'); try writeField(w, field.key_ptr.*); try w.print("\tF\t{d}\n", .{fn_id}); exports += 1;
                    },
                    .module_export => |m| {
                        try w.writeAll("E\t"); try w.writeAll(id); try w.writeByte('\t'); try writeField(w, field.key_ptr.*); try w.writeAll("\tM\t"); try writeField(w, m.module); try w.writeByte('\t'); try writeField(w, m.name); try w.writeByte('\n'); exports += 1;
                    },
                    else => {},
                };
            },
            .module => |m| { try w.writeAll("R\t"); try w.writeAll(id); try w.writeByte('\t'); try writeField(w, m); try w.writeByte('\n'); },
            else => {},
        }
        b.deinit(); chunk.deinit();
        if (modules % 5000 == 0) { try w.flush(); std.debug.print("modules={d} functions={d} exports={d} dynamic={d}\n", .{ modules, functions, exports, dynamic }); }
    }
    try w.flush();
