...
fn execute(source: []const u8) !struct { program: ir.Program, values: []const exec.Value, arena: std.heap.ArenaAllocator } {
    ...
    return .{ .program = program, .values = values, .arena = arena };
}

test "closed literal fields become anonymous slots" {
    const source = "local t={}; t.foo=2; t.bar=3; return t.foo+t.bar";
    var result = try execute(source);
    defer result.program.deinit();
    defer result.arena.deinit();
    defer exec.Vm.freeResults(result.values);
    try std.testing.expectEqual(@as(f64, 5), result.values[0].number);
    try std.testing.expect(result.program.shapes.items.len != 0);
    try std.testing.expectEqual(@as(usize, 0), result.program.shapes.items[0].field_keys.items.len);
    _ = try simplify.run(std.testing.allocator, &result.program);
    for (result.program.strings.items) |text| {
        try std.testing.expect(!std.mem.eql(u8, text, "foo"));
        try std.testing.expect(!std.mem.eql(u8, text, "bar"));
    }
}
test "one SSA dynamic key becomes one exclusive choice cell" {
    const source =
    ...
    try std.testing.expectEqual(@as(u32, 1), stats.choice_slots);
    try std.testing.expectEqual(@as(u32, 1), stats.choice_reads);
    try std.testing.expectEqual(@as(u32, 1), stats.choice_writes);
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var vm = try exec.Vm.init(arena.allocator());
...
test "fresh object per loop iteration has one key not an expanding schema" {
    const a = std.testing.allocator;
    var chunk = try lua.parse(a, "local s=0; for i=1,8 do local t={};t[i]=i;s=s+t[i] end;return s");
    defer chunk.deinit();
    ...
    defer exec.Vm.freeResults(out);
    try std.testing.expectEqual(@as(f64, 36), out[0].number);
}
test "key changes without object recreation must preserve older keys" {
    const a = std.testing.allocator;
    var chunk = try lua.parse(a, "local t={}; for i=1,8 do t[i]=i end;return t[1],t[8]");
    defer chunk.deinit();
    var p = try ir.lowerChunk(a, &chunk);
    defer p.deinit();
    const stats = try run(a, &p);
