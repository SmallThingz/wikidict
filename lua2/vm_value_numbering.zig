...
test "value numbering eliminates repeated proven numeric expressions" {
    var chunk = try lua.parse(std.testing.allocator, "local x=0; for i=1,3 do x=x+i end; local a=x*2; local b=x*2; return a+b");
    defer chunk.deinit();
    var p = try ir.lowerChunk(std.testing.allocator, &chunk);
    defer p.deinit();
    _ = try flow.run(std.testing.allocator, &p);
    const stats = try run(std.testing.allocator, &p);
    _ = try cleanup.run(std.testing.allocator, &p);
    try verify.run(std.testing.allocator, &p);
    try std.testing.expect(stats.expressions != 0);
    var arena = std.heap.ArenaAllocator.init(std.testing.allocator);
    defer arena.deinit();
    var vm = try exec.Vm.init(arena.allocator());
    const out = try vm.executeRoot(&p, &.{});
    defer exec.Vm.freeResults(out);
    try std.testing.expectEqual(@as(f64, 24), out[0].number);
}
test "value numbering does not forward a captured memory cell through a snapshot" {
    const source = "local x=2; local function set() x=9 end; local y=x; set(); return x,y";
    ...
    const out = try vm.executeRoot(&p, &.{});
    defer exec.Vm.freeResults(out);
    try std.testing.expectEqual(@as(f64, 9), out[0].number);
