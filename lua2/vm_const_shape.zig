...
fn execute(source: []const u8) !struct { program: ir.Program, values: []const exec.Value, arena: std.heap.ArenaAllocator, stats: Stats } {
    ...
    try @import("vm_verify.zig").run(a, &program);
...
test "constant child table keeps compact literal and gains numeric slots" {
    var result = try execute("local e={numbers={}};e.numbers[1]=4;e.numbers[2]=5;return e.numbers[1],e.numbers[2]");
    ...
    try std.testing.expectEqual(@as(f64, 5), result.values[1].number);
    try std.testing.expect(result.stats.shaped_templates >= 1);
    try std.testing.expect(result.stats.slot_writes >= 2);
    try std.testing.expect(result.stats.slot_reads >= 2);
}
test "mutated constant parent field stays dynamic" {
    const a = std.testing.allocator;
    var chunk = try lua.parse(a, "local e={numbers={}};e.numbers={};e.numbers[1]=5;return e.numbers[1]");
    defer chunk.deinit();
    var program = try ir.lowerChunk(a, &chunk);
    ...
    var indexes: u32 = 0;
    for (program.functions.items) |maybe| if (maybe) |function| {
        for (function.insts.items) |inst| indexes += @intFromBool(inst.op == .get_index or inst.op == .set_index or inst.op == .table_set);
    };
    try std.testing.expect(indexes >= 2);
    var arena = std.heap.ArenaAllocator.init(a);
    defer arena.deinit();
    var vm = try exec.Vm.init(arena.allocator());
...
test "unknown parent mutation prevents child identity propagation" {
    const a = std.testing.allocator;
    var chunk = try lua.parse(a, "local k=...;local e={numbers={}};e[k]={};e.numbers[1]=5;return e.numbers[1]");
    defer chunk.deinit();
    var program = try ir.lowerChunk(a, &chunk);
    defer program.deinit();
    const stats = try run(a, &program);
    try std.testing.expectEqual(@as(u64, 0), stats.slot_writes);
}

test "escaping parent prevents stale child identity propagation" {
    const a = std.testing.allocator;
    var chunk = try lua.parse(a, "local function replace(e)e.numbers={}end;local e={numbers={}};replace(e);e.numbers[1]=5;return e.numbers[1]");
    defer chunk.deinit();
    var program = try ir.lowerChunk(a, &chunk);
    defer program.deinit();
    const stats = try run(a, &program);
...
test "shaped constant templates still materialize distinct mutable children" {
    var result = try execute("local function make(x)local e={numbers={}};e.numbers[1]=x;return e.numbers end;local a=make(3);local b=make(4);a[1]=9;return a[1],b[1],a==b");
    defer result.program.deinit();
    defer result.arena.deinit();
    defer exec.Vm.freeResults(result.values);
    try std.testing.expectEqual(@as(f64, 9), result.values[0].number);
    try std.testing.expectEqual(@as(f64, 4), result.values[1].number);
