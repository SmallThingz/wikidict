...
const refs = @import("../compiler/ref.zig");
const global_abi = @import("../abi/globals.zig");
const graph_mod = @import("../compiler/graph.zig");
const sem = @import("../compiler/semantics.zig");
const aot_hint = @import("../compiler/aot_hint.zig");
const static_fields = @import("../abi/static_fields.zig");
const shape_key = @import("../abi/shape_key.zig");
const aot_data = @import("program_data.zig");
...
fn emitSimple(out: *std.ArrayList(u8), a: A, p: *const ir.Program, function: *const ir.Function, plan: *const FunctionPlan, inst: ir.Inst, pc: usize, stats: *Stats, range: ?FunctionRange) !void {
    switch (inst.op) {
        ...
        .load_number => {
            ...
            const number = try @import("../compiler/numbers.zig").parse(p.strings.items[inst.aux]);
...
test "finalized IR emits native Zig directly" {
    const lua = @import("../parser/root.zig");
    const opt = @import("../compiler/optimize.zig");
...
test "sharded module registry replaces constant-only roots with descriptors" {
    ...
    const link = @import("../compiler/link_image.zig");
    const allocator = std.testing.allocator;
