        ...
        for (block.start..block.end) |pc| {
            ...
            switch (inst.op) {
                .get_global_slot, .set_global_slot => {
                    const count = if (program.global_shape) |sid| program.shapes.items[sid].field_count else @import("../abi/globals.zig").count;
                ...
                .branch_compare => {
                    const op = try @import("semantics.zig").comparisonOpcode(inst.count);
...
pub fn run(allocator: std.mem.Allocator, program: *const ir.Program) !void {
    if (program.global_shape) |id| {
        if (id >= program.shapes.items.len or program.shapes.items[id].field_count < @import("../abi/globals.zig").count) return error.BadGlobalLayout;
    }
