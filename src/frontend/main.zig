...
fn run(init: std.process.Init) !u8 {
    ...
    if (opts.command == .catalog or opts.command == .install) {
        ...
        if (opts.command == .catalog) try library.catalog(init.io, init.gpa, opts.query, w) else try library.install(init.io, init.gpa, opts.root, opts.query, opts.sha256, w);
        try w.flush();
        return 0;
    }
